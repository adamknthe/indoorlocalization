...
class StepDetector {
  ...
  static double thresholdMultiplier =
      5; // Adjustable multiplier for adaptive thresholding
  static double stepLength = 0.76; // Default step length in meters
  int stepBuffer = 10; // Buffer for ignoring consecutive steps
  static int stepCount = 0;

  void processAccelerometerData(List<double> rawData) {
    // Apply low-pass filter (simple moving average)
    List<double> filteredData = lowPassFilter(rawData);

    // Compute acceleration magnitude
    List<double> magnitudeData = computeMagnitude(filteredData);

    // Apply adaptive thresholding
    double threshold = calculateAdaptiveThreshold(magnitudeData);

    // Detect steps using peak detection
    detectSteps(magnitudeData, threshold);
  }

  List<double> lowPassFilter(List<double> data) {
    double alpha = 0.8;
    List<double> filteredData = [data[0]];

    for (int i = 1; i < data.length; i++) {
      filteredData.add(alpha * filteredData[i - 1] + (1 - alpha) * data[i]);
    }

    return filteredData;
  }

  List<double> computeMagnitude(List<double> data) {
    List<double> magnitudeData = [];

    for (int i = 0; i < data.length; i += 3) {
      double magnitude = sqrt(data[i] * data[i] +
          data[i + 1] * data[i + 1] +
          data[i + 2] * data[i + 2]);
  ...
  double calculateAdaptiveThreshold(List<double> data) {
    double mean = data.reduce((a, b) => a + b) / data.length;
    double standardDeviation = sqrt(
        data.map((x) => pow(x - mean, 2)).reduce((a, b) => a + b) /
            data.length);

    return mean + thresholdMultiplier * standardDeviation;
  }

  void detectSteps(List<double> data, double threshold) {
    for (int i = 1; i < data.length - 1; i++) {
      if (data[i] > threshold &&
          data[i] > data[i - 1] &&
          data[i] > data[i + 1] &&
          stepBuffer <= 0) {
        // Additional criteria for step confirmation can be added here

        stepCount++;
        stepBuffer = 10; // Reset step buffer
      } else {
        stepBuffer--;
      }
    }
  ...
  double estimateStrideLength() {
    // Implement logic to estimate stride length based on user characteristics
    return stepLength;
  }
}
