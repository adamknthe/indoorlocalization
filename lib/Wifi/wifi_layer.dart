...
class WifiLayer{
  ...
  WifiLayer({required this.referencePoints, required this.Buildingname, required this.floorLevel, required this.GeojsonOfOutline});
  ...
  static bool isInsideOfBuildingFromList(List<LatLng> latlng, Posi position){
    return PolygonUtil.containsLocation(LatLng(position.y, position.x),latlng,true);
  }
  ...
  static Future<void> createReferencePoints(WifiLayer wifiLayer) async {
    Posi root = getCenter(getListFromPolygonOutline(wifiLayer.GeojsonOfOutline));
  ...
  static List<ReferencePoint> generateMatrix(Posi center, int gridSize){
    ...
    for(int i = centerxy; i < gridSize; i++){
      for(int j = centerxy; j < gridSize; j++){
       matrixPosi[i][j]= Posi(x: (center.x+x*(i-centerxy)), y: center.y+y*(j-centerxy)) ;
      }
    }
    // down rigth
    ...
    for(int i = 0; i < gridSize; i++){
      for(int j = 0; j < gridSize; j++){
        if(isInsideOfBuildingFromList(latlng, matrixPosi[i][j])){
          ReferencePoint referencePoint = ReferencePoint(latitude: matrixPosi[i][j].y, longitude: matrixPosi[i][j].x, accesspoints: [], accesspointsNew: [],documentId: "");
  ...
  Future<bool> createWifiLayer() async{
    ...
    try{
      ...
      for(int i= 0; i < referencePoints.length; i++){
        ReferencePoint? ref = await ReferencePoint.createReferencePoint(latitude: referencePoints[i].latitude, longitude: referencePoints[i].longitude, accesspoints: referencePoints[i].accesspoints, accesspointsNew: referencePoints[i].accesspointsNew);
  ...
  static Future<WifiLayer?> getWifiLayer(String building, int floor) async {
    try {
      DocumentList result = await Runtime.database.listDocuments(
          databaseId: databaseIdWifi,
          collectionId: collectionIDMar,
          queries: [Query.equal("name", building),Query.equal("floor", floor)]
      );
