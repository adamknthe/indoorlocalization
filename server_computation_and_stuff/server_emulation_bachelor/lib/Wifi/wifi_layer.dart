...
class WifiLayer {
  ...
  static List<LatLng> getListFromPolygonOutlinefloor(String json) {
    List<LatLng> floor = [];
    GeoJSON geoJSON = GeoJSON.fromJSON(json);
    //print(json);
    List<List<double>> coordinates = geoJSON.toMap()["features"][28]["geometry"]["coordinates"][0];
    print(geoJSON.toMap()["features"][28]["properties"]["name"]);
    int i = 0;
    while (coordinates.length > i) {
      floor.add(LatLng(coordinates[i][1],coordinates[i][0]));
      i++;
...
class WifiLayerGetter{
  ...
  static Future<bool> getFirstLayer() async{
    ...
    //try{
      ...
      print(wifiLayer?.Buildingname.toString());
